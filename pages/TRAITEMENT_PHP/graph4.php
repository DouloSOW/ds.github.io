...
$chaine.="
        var myChart = Highcharts.chart('container2', {
            ...
            title: {
                text: 'Evolution du nombre total de cas positifs, de cas gueris et de cas sous traitement'
            },
            xAxis: {
                categories: [  ";
...
$chaine.="  },yAxis: {
            ...
            series: [{
                ...
                data: [";
$resultat=$conn-> query ("SELECT cumul_cas_positifs from V_2");
//if (! $resultat) { echo "Erreur requete"; exit;} 
...
while($row=$resultat-> fetch_assoc())
{
    $ydata1[$i]=intval($row["cumul_cas_positifs"]);
	$chaine.=$ydata1[$i].",";
	$i++;
}
$chaine.="],color: '#FF0000'
            }, {
                name: 'Nombre total de cas gueris',
                data: [";
$resultat=$conn-> query ("SELECT cumul_cas_guerri  from V_2");
//if (! $resultat) { echo "Erreur requete"; exit;} 
...
while($row=$resultat-> fetch_assoc())
{
	$ydata2[$i]=intval($row["cumul_cas_guerri"]);
	$chaine.=$ydata2[$i].",";
	$i++;
}
$chaine.="],";
$chaine.="color: '#00FF00'
               ...
               name: 'Nombre total de cas sous traitement',
                data: [";
$resultat=$conn-> query ("SELECT cumul_cas_sous_taitements  from V_2");
//if (! $resultat) { echo "Erreur requete"; exit;} 
...
while($row=$resultat-> fetch_assoc())
{
	$ydata3[$i]=intval($row["cumul_cas_sous_taitements"]);
	$chaine.=$ydata3[$i].",";
	$i++;
}
$chaine.="],";
