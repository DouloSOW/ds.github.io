...
$chaine.="
        var myChart = Highcharts.chart('container10', {
            ...
            title: {
                text: 'Nouveaux guerisons par jour'
            },
            xAxis: {
                categories: [  ";
...
$chaine.="  },yAxis: {
            ...
            series: [{
				name: 'Nouveaux guerisons',
                data: [";
$resultat=$conn-> query ("SELECT cas_guerri from V_2");
//if (! $resultat) { echo "Erreur requete"; exit;} 
...
while($row=$resultat-> fetch_assoc())
{
    $ydata1[$i]=intval($row["cas_guerri"]);
	$chaine.=$ydata1[$i].",";
	$i++;
}
$chaine.="],color: '#00FF00'
