...
$chaine.="
        var myChart = Highcharts.chart('container1', {
            ...
            title: {
                text: 'Evolution du mode propagation de la maladie'
            },
            xAxis: {
                categories: [  ";
...
$chaine.="  },yAxis: {
            ...
            series: [{
				 name: 'Nombre total de transmissions communautaires',
                data: [";
$resultat=$conn-> query ("SELECT cumul_trans_com  from V_2");
$i=0;
while($row=$resultat-> fetch_assoc())
{
	$ydata3[$i]=intval($row["cumul_trans_com"]);
	$chaine.=$ydata3[$i].",";
	$i++;
}
$chaine.="],color: '#FF0000'
            }, {
                name: 'Nombre total de cas contact',
                data: [";
$resultat=$conn-> query ("SELECT cumul_cas_contact  from V_2");
$i=0;
while($row=$resultat-> fetch_assoc())
{
	$ydata2[$i]=intval($row["cumul_cas_contact"]);
	$chaine.=$ydata2[$i].",";
	$i++;
}
$chaine.="],";
$chaine.="color: '#3300CC'
               ...
               name: 'Nombre total de cas importes',
                data: [";
$resultat=$conn-> query ("SELECT cumul_cas_importes from V_2");
$i=0;
while($row=$resultat-> fetch_assoc())
{
	$ydata1[$i]=intval($row["cumul_cas_importes"]);
	$chaine.=$ydata1[$i].",";
	$i++;
}
$chaine.="],";
