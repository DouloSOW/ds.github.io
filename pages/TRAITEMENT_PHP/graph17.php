...
$chaine.="
        var myChart = Highcharts.chart('container17', {
            chart: {
                type: 'line', 
				zoomType: 'x' 
            },
            title: {
                text: 'Evolution du nombre total de deces'
            },
            xAxis: {
                categories: [  ";
...
$chaine.="  },yAxis: {
            ...
            series: [{
				name: 'Nombre total de deces',
                data: [";
$resultat=$conn-> query ("SELECT cumul_deces from V_2");
//if (! $resultat) { echo "Erreur requete"; exit;} 
...
while($row=$resultat-> fetch_assoc())
{
    $ydata1[$i]=intval($row["cumul_deces"]);
	$chaine.=$ydata1[$i].",";
	$i++;
}
$chaine.="],color: '#FF0000'
